...
openssl genpkey -algorithm ED25519 -out root_keypair.pem -quiet
...
openssl x509 -req -in root_csr.pem -copy_extensions copyall -signkey root_keypair.pem -days 7 -out root_cert.pem

openssl x509 -in root_cert.pem -noout -text -quiet

openssl genpkey -algorithm ED25519 -out intermediate_keypair.pem -quiet
...
openssl x509 -req -in intermediate_csr.pem -copy_extensions copyall -CA root_cert.pem -CAkey root_keypair.pem -days 7 -out intermediate_cert.pem

openssl x509 -in intermediate_cert.pem -noout -text -quiet

openssl genpkey -algorithm ED25519 -out leaf_keypair.pem -quiet

openssl req -new -subj "/CN=Leaf" -addext "basicConstraints=critical,CA:FALSE" -key leaf_keypair.pem -out leaf_csr.pem

openssl x509 -req -in leaf_csr.pem -copy_extensions copyall -CA intermediate_cert.pem -CAkey intermediate_keypair.pem -days 7 -out leaf_cert.pem
